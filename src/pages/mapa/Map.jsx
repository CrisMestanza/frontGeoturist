...
const RANGOS_PRECIO = [
  { label: "Gratuito", value: "0-0" },
  { label: "S/. 1 – 10", value: "1-10" },
  { label: "S/. 10 – 30", value: "10-30" },
  { label: "S/. 30 – 60", value: "30-60" },
  { label: "S/. 60 a más", value: "60-999999" },
];

const buildPriceMarkerIcon = (precio, isHovered = false) => {
  if (precio == null) return null;
  const label = `S/${Math.round(precio)}`;
  const w = Math.max(56, label.length * 9 + 22);
  const h = 28;
  const fill = isHovered ? "#0ea66b" : "#1a2e4a";
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h + 7}">
    <rect x="1.5" y="1.5" rx="12" ry="12" width="${w - 3}" height="${h - 3}" fill="${fill}" stroke="white" stroke-width="1.5"/>
    <text x="${w / 2}" y="16" font-family="Arial,sans-serif" font-size="11" font-weight="700" fill="white" text-anchor="middle">${label}</text>
    <polygon points="${w / 2 - 5},${h - 2} ${w / 2 + 5},${h - 2} ${w / 2},${h + 6}" fill="${fill}"/>
  </svg>`;
...
function MyMap() {
  ...
  const [hideGoogleLabels, setHideGoogleLabels] = useState(true);
  const [mapTypeMenuFor, setMapTypeMenuFor] = useState(null);
  const [hoveredProyecto, setHoveredProyecto] = useState(null);
  ...
  const loadMapProjects = async ({
    ...
    if (categoria) query.set("categoria", String(categoria));
  ...
  const onResizerMouseDown = useCallback((e) => {
    ...
    resizerStartWidthRef.current = panelWidth;
  ...
  useEffect(() => {
    const onMouseMove = (e) => {
      if (!isResizingRef.current || !splitBodyRef.current) return;
      const totalWidth = splitBodyRef.current.getBoundingClientRect().width;
      const deltaX = e.clientX - resizerStartXRef.current;
      const newWidth = Math.min(
        Math.max(resizerStartWidthRef.current + (deltaX / totalWidth) * 100, 25),
        72,
      );
  ...
  const makePinSVG = (colorTop, colorBot, iconPath) => {
...
<linearGradient id="shine" x1="0%" y1="0%" x2="60%" y2="80%">
<stop offset="0%" stop-color="rgba(255,255,255,0.38)"/>
<stop offset="100%" stop-color="rgba(255,255,255,0)"/>
</linearGradient>
<filter id="drop" x="-40%" y="-20%" width="180%" height="180%">
<feDropShadow dx="0" dy="5" stdDeviation="5" flood-color="rgba(0,0,0,.32)"/>
</filter>
</defs>
<path d="M28,3 C13,3 3,13 3,28 C3,37 8,45 16,50 L28,70 L40,50 C48,45 53,37 53,28 C53,13 43,3 28,3Z" fill="url(#g)" filter="url(#drop)"/>
<path d="M28,3 C13,3 3,13 3,28 C3,37 8,45 16,50 L28,70 L40,50 C48,45 53,37 53,28 C53,13 43,3 28,3Z" fill="url(#shine)"/>
<circle cx="28" cy="27" r="16" fill="rgba(255,255,255,0.18)"/>
<g transform="translate(28,27)" fill="white" stroke="white" stroke-linejoin="round" stroke-linecap="round">${iconPath}</g>
</svg>`;
    return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}`;
  };

  /* Iconos por categoría — blancos, centrados en (0,0), radio ≈12px */
  const PIN_ICONS = {
    // Piscinas – dos olas + cabeza nadador
    1: makePinSVG("#0ea5e9", "#0369a1",
      `<path d="M-12,-4 Q-6,4 0,-4 Q6,-12 12,-4" fill="none" stroke="white" stroke-width="3" stroke-linecap="round"/><path d="M-12,5 Q-6,13 0,5 Q6,-3 12,5" fill="none" stroke="white" stroke-width="3" stroke-linecap="round"/><circle cx="0" cy="-13" r="3.5" fill="white" stroke="none"/>`),
    // Cataratas – gota grande con reflejo
    2: makePinSVG("#06b6d4", "#0e7490",
      `<path d="M0,-14 C7,-8 13,0 0,12 C-13,0 -7,-8 0,-14Z" stroke-width="0"/><ellipse cx="-3" cy="-4" rx="2.5" ry="4" fill="rgba(255,255,255,0.45)" stroke="none" transform="rotate(-30,-3,-4)"/>`),
    // Recreos – árbol pino con tronco
    3: makePinSVG("#16a34a", "#14532d",
      `<path d="M0,-14 L10,0 L6,0 L9,8 L-9,8 L-6,0 L-10,0Z" stroke-width="0"/><rect x="-3" y="8" width="6" height="5" rx="1" stroke="none" fill="rgba(255,255,255,0.7)"/>`),
    // Miradores – montaña con sol
    4: makePinSVG("#f59e0b", "#b45309",
      `<path d="M-13,9 L-4,-6 L2,1 L7,-8 L14,9Z" stroke-width="0"/><circle cx="-8" cy="-9" r="4.5" fill="white" stroke="none"/><path d="M-8,-15 L-8,-13 M-2,-9 L-4,-9 M-14,-9 L-12,-9 M-5.5,-13.5 L-6.5,-12.5 M-10.5,-13.5 L-9.5,-12.5" fill="none" stroke="white" stroke-width="1.8" stroke-linecap="round"/>`),
    // Aventura – rayo con estela
    5: makePinSVG("#f97316", "#c2410c",
      `<path d="M4,-14 L-6,1 L1,1 L-4,14 L10,-1 L3,-1Z" stroke-width="0"/>`),
  };

  const DEFAULT_PIN = makePinSVG("#0d9488", "#134e4a",
    `<path d="M0,-14 C6,-9 13,0 0,10 C-13,0 -6,-9 0,-14Z" stroke-width="0"/><ellipse cx="-3" cy="-5" rx="3" ry="5" fill="rgba(255,255,255,0.35)" stroke="none" transform="rotate(-25,-3,-5)"/>`);

  const getProjectIconUrl = (p) => {
    if (filtroBotActivo && p.iconoTipo !== "casa") return DEFAULT_PIN;
    const idCat = Number(p.idcategoria);
    if (idCat && PIN_ICONS[idCat]) return PIN_ICONS[idCat];
    return DEFAULT_PIN;
  };
  ...
  const handleCategoriaChange = (categoriaId) => {
    const idNumerico = categoriaId === "" ? "" : Number(categoriaId);
    if (selectedCategoria === idNumerico) {
      setSelectedCategoria("");
    } else {
      setSelectedCategoria(idNumerico);
    }
    setFiltroBotActivo(false);
  };
  ...
  useEffect(() => {
    ...
    if (selectedTipo || selectedRango || selectedCategoria) {
      setFiltroBotActivo(false);
    }
  }, [filtroBotActivo, selectedTipo, selectedRango, selectedCategoria]);
  ...
  return (
    <div className={styles.container}>
      <header ref={headerRef} className={`${styles.cabecera} `}>
        ...
        <Link to="/" className={styles.logoContainer} aria-label="Ir a inicio">
          ...
          <span className={styles.brandName}>
            ...
            <span className={styles.habita}>Turist</span>
          </span>
        </Link>
        ...
        >
          ...
          {
            <>
              {/* SELECT PERSONALIZADO: CATEGORÍA */}
              <CustomSelect
                label="QUIERO VER"
              ...
              {/* SELECT PERSONALIZADO: PRECIO ENTRADA */}
              <CustomSelect
      ...
      >
        ...
        {/* ── Panel izquierdo: lista de ofertas o detalle de proyecto ── */}
        <div
          className={styles.ofertasPanel}
          style={{ width: `${panelWidth}%`, minWidth: "280px", maxWidth: "72%" }}
        >
          {!isMobileViewport && selectedProyecto ? (
            <ProyectoSidebar
          ...
          <div className={styles.ofertasHeader}>
            <span className={styles.ofertasHeaderCount}>
              <span className={styles.ofertasHeaderBadge}>{visibleProyectos.length}</span>
              centro{visibleProyectos.length !== 1 ? "s" : ""} turístico{visibleProyectos.length !== 1 ? "s" : ""}
            </span>
          </div>
          {visibleProyectos.length === 0 ? (
            <div className={styles.ofertasEmpty}>
              <span className={styles.ofertasEmptyIcon}>🗺️</span>
              No hay centros turísticos en esta área.
              <br />Mueve el mapa para explorar.
            </div>
          ) : (
            <div className={styles.ofertasList}>
              {visibleProyectos.map((p) => {
                const precio = p.precio_entrada ?? p.precio;
                const isHov = hoveredProyecto === p.idproyecto;
                return (
                  <button
                    key={p.idproyecto}
                    type="button"
                    className={`${styles.ofertaCard} ${isHov ? styles.ofertaCardHovered : ""}`}
                    onClick={() => handleMarkerClick(p)}
                    onMouseEnter={() => setHoveredProyecto(p.idproyecto)}
                    onMouseLeave={() => setHoveredProyecto(null)}
                  >
                    {/* Imagen con overlays */}
                    <div className={styles.ofertaCardImgWrap}>
                      {p.imagen_principal ? (
                        <img
                          className={styles.ofertaCardImg}
                          src={p.imagen_principal?.replace(/^https?:\/\/[^/]+/, "")}
                          alt={p.nombreproyecto}
                          loading="lazy"
                        />
                      ) : (
                        <div className={styles.ofertaCardImgPlaceholder}>🏔️</div>
                      )}
                      <div className={styles.ofertaCardImgOverlay} />
                      {p.categoria?.nombre && (
                        <span className={styles.ofertaCardCategoriaBadge}>
                          {p.categoria.nombre}
                        </span>
                      )}
                      {precio != null && (
                        <span className={styles.ofertaCardPrecioBadge}>
                          S/{Math.round(precio)}
                        </span>
                      )}
                    </div>
                    {/* Info */}
                    <div className={styles.ofertaCardBody}>
                      <span className={styles.ofertaCardNombre}>{p.nombreproyecto}</span>
                      {p.descripcion && (
                        <span className={styles.ofertaCardDesc}>{p.descripcion}</span>
                      )}
                    </div>
                    <div className={styles.ofertaCardFooter}>
                      <span className={styles.ofertaCardVerMas}>Ver destino →</span>
                    </div>
                  </button>
                );
              })}
        ...
        {!isMobileViewport && (
          ...
          >
            <span className={styles.panelResizerDots} />
          </div>
        )}

        {/* ── Panel derecho: mapa ── */}
      <div
        className={`${styles.mapViewport} ${styles.mapViewportSplit}`}
      >
                  ...
                  {visibleProyectos.map((p) => (
                    <Marker
                      ...
                      icon={{
                        ...
                        scaledSize: new window.google.maps.Size(52, 67),
                        anchor: new window.google.maps.Point(26, 67),
                      }}
          ...
          {visibleProyectos.length > 0 && !ENABLE_PROJECT_CLUSTERING && (
            <>
              {visibleProyectos.map((p) => {
                const precio = p.precio_entrada ?? p.precio;
                const isHov = hoveredProyecto === p.idproyecto;
                const priceIconUrl = buildPriceMarkerIcon(precio, isHov);
                if (priceIconUrl) {
                  const w = Math.max(56, `S/${Math.round(precio)}`.length * 9 + 22);
                  return (
                    <Marker
                      key={p.idproyecto}
                      ...
                      icon={{
                        url: priceIconUrl,
                        scaledSize: new window.google.maps.Size(w, 35),
                        anchor: new window.google.maps.Point(w / 2, 35),
                      }}
                      title={p.nombreproyecto}
                      onClick={() => handleMarkerClick(p)}
                    />
        ...
        >
          <div className={styles.mapTypeTabs}>
            ...
            <button
              type="button"
              className={`${styles.mapTypeBtn} ${hideGoogleLabels ? styles.mapTypeBtnActive : ""}`}
              onClick={() => setHideGoogleLabels((prev) => !prev)}
              aria-pressed={hideGoogleLabels}
              title={hideGoogleLabels ? "Mostrar etiquetas del mapa" : "Ocultar etiquetas del mapa"}
            >
              Etiquetas
            </button>
          </div>
      ...
      {selectedProyecto && isMobileViewport && (
        <ProyectoSidebar
          inmo={selectedProyecto?.inmo}
          proyecto={selectedProyecto}
          selectedLote={selectedLote?.lote || null}
